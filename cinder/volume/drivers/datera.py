...
class DateraDriver(san.SanISCSIDriver):
    ...
    def _update_cluster_stats(self):
        ...
        stats = {
            ...
            'total_capacity_gb': int(results['total_capacity']) / units.GiB,
            'free_capacity_gb': int(results['available_capacity']) / units.GiB,
        ...
        if not response.ok:
            ...
            if response.status_code == 404:
                ...
                msg = 'Request to Datera cluster returned bad status:' \
                      ' %(status)s | %(reason)s' % {'status': response.status_code,
